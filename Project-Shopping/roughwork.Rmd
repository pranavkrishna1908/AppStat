...
ggplot()+
  geom_bar(aes(x = Data$weekend, colour = as.factor(y)))
...
model3 = glm(formula = purchase ~ product_related_duration + exit_rates + page_values + month + visitor_type + special_day + I(special_day^2)- 1, data = Data, family = binomial)
AUC_eval(model3,Data)#Rejected
#try without quadratic term

model3 = glm(formula = purchase ~ product_related_duration + exit_rates + page_values + month + visitor_type + special_day - 1, data = Data, family = binomial)
AUC_eval(model3,Data)
#doesnt help
#give up on special day
...
#even this does not improve the auc

modeltemp = glm(formula = purchase ~ product_related_duration + exit_rates + 
                 page_values + month - 1 + n_admin + n_info, family = binomial, data = Data)
AUC_eval(modeltemp,Data)#we see the auc

#try a few models by increasing the number of covariates
AUC_eval(glm(formula = purchase ~ product_related_duration + exit_rates +        page_values + month - 1 + n_admin + n_info+ bounce_rates , family = binomial, data = Data),Data)#we see the 
...
```{r}
AUC_eval(glm(formula = purchase ~ product_related_duration + exit_rates + page_values + month - 1 + n_admin + n_info*time_info+ bounce_rates , family = binomial, data = Data),Data)#we see the 
gm0  = glm(formula = purchase ~ product_related_duration + exit_rates + page_values + month - 1 + n_admin + n_info*time_info+ bounce_rates , family = binomial, data = Data)
```
 
```{r}
Data %>%
mutate(res=resid(gm0),operating_systems=as.numeric(operating_systems), browser=as.numeric(browser), #region = as.numeric(region), 
