...
percapita = apply(cumultaive , 1, function(row) row/populations)
final = apply(cumultaive,2, function(col) predict(smooth.spline(col))$y )
...
## now, make plts
```{r}
plot(logged_cmulative[,5], type = 'l')
for(i in 2:51){
  points(logged_cmulative[,i], col = i, type = 'l')
}
```
